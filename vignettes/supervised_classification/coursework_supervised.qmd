...
The Area Under the Curve (ACU) is an index that determines how well the model is. The scale goes from 0-1 where 1 is perfect and 0.5 is random and anything less than 0.5 is worse than truly random.

![](../../data_cache/vignettes/supervised_classification/roc.html){width="100%" height="600px"}

Here it can be seen that the performance of the LDA classifier has an AUC of 0.19 and the QDA classifier as a AUC of 0.21. This means that both of these models are worse at separating between the two groups than if the model was truly random. However QDA is slightly better.
